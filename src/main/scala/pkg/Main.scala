...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    s"$intMonad1 $intMonad2 $intMonad3 $intMonad7".traceStdOut
    
    "Now with do-while notation (for-yield in Scala is like do-while in Haskell)".traceStdOut
    val intMonad7redo = for {
      three <- intMonad3
      four <- IdentityMonad(three + 1)
    } yield three + four
    intMonad7redo.traceStdOut
    testStateMonad()
  }
  ...
  def testStateMonad(): Unit = {
...

    {
      ...
      s"$twoDouble $threeDouble $fourDouble".traceStdOut // 1.0 2.0 3.0
      s"$two $three $four".traceStdOut // 2 3 4
    }
